...
export function NameForgeApp() {
  ...
  useEffect(() => {
    if (!options.extensions.includes(extension) && options.extensions.length) {
      setExtension(options.extensions[0]);
  ...
  const verifyDomains = async (items: NameCandidate[]) => {
    ...
    try {
      const allResults: Record<string, NameCandidate["domains"][Extension]> = {};
      for (const selectedExtension of options.extensions) {
        const domains = batch.map((item) => `${item.name.toLowerCase()}${selectedExtension}`);
      ...
      const statusFor = (item: NameCandidate, selectedExtension: Extension) =>
        allResults[`${item.name.toLowerCase()}${selectedExtension}`] || "unknown";
      const preferredExtension: Extension = options.tone === "ai"
        ? ".ai"
      ...
      <div className="grid min-w-0 items-start gap-5 lg:grid-cols-[285px_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          <section className="overflow-hidden rounded-2xl border border-line bg-panel/90 shadow-glow">
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button disabled={verifying.size > 0} onClick={() => verifyDomains(filtered)} className="inline-flex items-center gap-2 rounded-xl border border-lime/25 bg-lime/10 px-3.5 py-2.5 text-xs font-semibold text-lime transition hover:bg-lime/15 disabled:opacity-50">
                  ...
                  {verifying.size ? "Checking all domains…" : "Verify top 25 domains"}
                </button>
            ...
            />
            <ResultsTable candidates={filtered.slice(0, visibleCount)} extensions={options.extensions} shortlisted={new Set(shortlist.map((item) => item.name))} onToggleShortlist={toggleShortlist} sort={sort} descending={descending} onSort={changeSort} onVerify={(candidate) => verifyDomains([candidate])} verifying={verifying} />
